
import {createI18n} from 'vue-i18n'

export const i18n = createI18n({
    legacy: false,
    locale: 'es',
    messages: {
        en: {
            welcome: 'Welcome ',
            Config_Page: 'Config Page ',
            description_1: 'Systems Engineer',
            description_2: 'my name is',
            Rigo:'Rigoberto',
            Spanish: 'Spanish',
            config: 'Configuracion',
            English: 'English',
            Save: 'Save',
            Language: 'Language',
            Text_Color: 'Text Color',
            Navigation: 'Navigation',
            SoftEng: 'Systems Engineer',
            description_3: 'I design and code beautifully simple things and i love what i do. Just simple like that!',
            years_experience: 'Years of experience',
            completed_projects: 'Completed Projects',
            Deployment_Hosting: 'Deployment and Hosting',
            Database_Design: 'Database Design',
            Web_Development: 'Web Development',
            techDes: 'Technologies with which I have worked',
            Web_Desc: 'Web development is the work involved in developing a website for the Internet or an intranet.',
            Databasw_Desc: 'Database design is the process of producing a detailed data model of a database.',
            languajes: 'Languajes',
            Deploy_Desc: 'Deployment and hosting refers to the process of publishing a web application so that it is accessible to the world.',
            Contact: 'Contact Me',
            D_CV: 'Download CV',
            View_LK: 'View LinkedIn Profile',
            View_GH: 'View GitHub Profile',
            My_I: 'Languages',
            Native: 'Native',
            HistoryExp: 'Education and Work Experience',
            Describe_P: 'Describe your project!!',
            Send_Message: 'Send me a message!',
            Tech: 'Technologies',
            Ed_Exp: 'Education and Work Experience',
            Pro_Lenguajes: 'Programming Languages',
            Services: 'Services',
            Send: 'Send',
            Success_Message: 'Message Sent Successfully, I will contact you soon!',
            Success: 'Message Sent',
            Error_Message: 'Error sending message, try again!',
            Error: 'Error',
            Software_Developer: 'Software Developer',
            Proyect_Uni: 'University Projects',
            Proyect_Uni_D: 'Creation of e-commerce platforms with .net, vue nest js and mongodb, development of applications with integrated maps in ionic and react native, integration of wompi, paypal payment platforms and third-party api. Deployments to production with heroku and railway, use of azure and aws cloud services.',
            Computer_Systems_Engineer: 'Computer Systems Engineer',
            Computer_Systems_Engineer_D: 'Having graduated from this career, I have become an integral engineer, with the skills to carry out strategic projects based on information technologies and their integration into the digital world.',
            About_Me: 'Student Graduated from the faculty of engineering with more than two years of experience. Passionate about software development with strong background in web and mobile application development, database design and application deployment.'
        },
        es: {
            Proyect_Uni: 'Proyectos Universitarios',
            completed_projects: 'Proyectos Completados',
            Proyect_Uni_D: 'Creacion de plataformas e-comerce con .net, vue nest js y mongodb, desarrollo de aplicaiones con mapas integrados en ionic y react native , integracion de plataformas de pago wompi, paypal y api de terceros. Despliegues a produccion con heroku y railway, uso de servicios en la nube azure y aws.',
            Software_Developer: 'Desarrollador de Software',
            Computer_Systems_Engineer: 'Ingeniería de Sistemas Computacionales',
            Computer_Systems_Engineer_D: 'Al haber Egresado esta carrera, me he convertido en un ingeniero integral, con las habilidades para llevar a cabo proyectos estratégicos basados en las tecnologías de la información y su integración en el mundo digital.',
            welcome: 'Bienvenido ',
            D_CV: 'Descargar CV',
            description_1: 'Ingeniero en Sistemas',
            Success_Message: 'Mensaje Enviado Exitosamente, Te contactare pronto!',
            Success: 'Mensaje Enviado',
            Error_Message: 'Error al enviar el mensaje, intenta de nuevo!',
            Error: 'Error',
            description_2: 'mi nombre es ',
            languajes: 'Lenguajes',
            Rigo:'Rigoberto',
            config: 'Configuracion',
            Config_Page: 'Configuracion de pagina ',
            Spanish: 'Español',
            English: 'Ingles',
            Save: 'Guardar',
            Tech: 'Tecnologias',
            Ed_Exp: 'Educacion y Experiencia Laboral',
            Pro_Lenguajes: 'Lenguajes de Programacion',
            Language: 'Idioma',
            Text_Color: 'Color de Texto',
            Navigation: 'Navegacion',
            SoftEng: 'Ingeniero en Sistemas',
            description_3: 'Diseño y codifico cosas simples y me encanta cunado lo hago. ¡Asi de simple!',
            years_experience: 'Años de experiencia',
            Deployment_Hosting: 'Despliegue y Hosting',
            Database_Design: 'Diseño de Base de Datos',
            Web_Development: 'Desarrollo Web',
            techDes: 'Tecnologias con las que Trabajo',
            Web_Desc: 'El desarrollo web es el trabajo involucrado en el desarrollo de un sitio web para Internet o una intranet.',
            Databasw_Desc: 'El diseño de bases de datos es el proceso de producir un diseño detallado de la base de datos.',
            Deploy_Desc: 'El despliegue y el hosting se refieren al proceso de publicar una aplicación web para que sea accesible para el mundo.',
            Contact: 'Contactame',
            HistoryExp: 'Educación y Experiencia Laboral',
            My_I: 'Idiomas',
            Native: 'Nativo',
            Send: 'Enviar',
            Describe_P: 'Describe tu proyecto!!',
            View_LK: 'Ver Perfil de LinkedIn',
            View_GH: 'Ver Perfil de GitHub',
            Services: 'Servicios',
            Send_Message: 'Enviame un mensaje!',
            About_Me: 'Estudiante Egresado de la facultad de ingeniería con más de dos años de experiencia. Apasionado por el desarrollo de software con sólidos conocimientos  en el desarrollo de aplicaciones web y móviles, diseño de bases de datos y despliegue de aplicaciones.',
        }
    }
})